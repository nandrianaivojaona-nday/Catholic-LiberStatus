...
export const StatisticsService = {
  ...
  getDashboardStats(parish: Territory, appData: MockData) {
    ...
    const zoneBreakdown = parish.zones?.map(zone => {
      ...
      // FIX: Correctly access leader ID from execTeam as per mockData structure.
  ...
  buildLiberstatusTree(parish: Territory, appData: MockData): Territory {
    ...
    parishNode.children = parish.zones?.map(zone => {
      ...
      // FIX: Removed the creation of the 'leadership' property which had the wrong data structure.
      // The `execTeam` property is already correctly spread from the `zone` object.
  ...
  // FIX: Add missing function 'getAllApvIds'
  // Helper to get all APV IDs within a zone
  getAllApvIds(zone: Territory): string[] {
    return zone.apvs?.map(apv => apv.id) || [];
  },

  // FIX: Add missing function 'getBelieversInTerritory'
