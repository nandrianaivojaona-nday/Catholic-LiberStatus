...
export const hasPermission = (user: User | null, action: string, options: PermissionOptions = {}): boolean => {
  ...
  switch (action) {
    ...
    case 'EDIT_PARISH_DETAILS':
        ...
        // The direct Parish Leader or any Priest can edit parish details.
        return (user.role === ROLES.parish_leader && isDirectLeader) || user.role === ROLES.priest;
    ...
    // --- Viewing Permissions ---
    case 'viewFamilies':
      // Any logged-in user can see the family structure, but not necessarily edit.
      return !!user;
